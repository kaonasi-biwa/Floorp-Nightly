 ...
 let {
  ...
  const bmsController = {
    eventFunctions: {
      ...
      sidebarItemMouse: {
        mouseOver: (event) => Services.obs.notifyObservers({
          eventType: "mouseOver",
          id: event.target.id
        }, "obs-panel"),
        mouseOut: (event) => Services.obs.notifyObservers({
          eventType: "mouseOut",
          id: event.target.id
        }, "obs-panel"),
    ...
    controllFunctions: {
      ...
      makeWebpanel: (webpanel_id) => {
        ...
        if (webpanobject != null && (
            (!(webpanobject?.getAttribute("changeuseragent") == "" && !webpanel_userAgent) && !(webpanobject?.getAttribute("usercontextid") == "" && wibpanel_usercontext == 0) && ((webpanobject?.getAttribute("changeuseragent") ?? "false") !== String(webpanel_userAgent) || (webpanobject?.getAttribute("usercontextid") ?? "0") !== String(wibpanel_usercontext))) || (
              ((webpanobject.className.includes("isFloorp") || webpanobject.className.includes("isTST")) && isWeb) || ((webpanobject.className.includes("isFloorp") || webpanobject.className.includes("isWeb")) && isTST) || ((webpanobject.className.includes("isTST") || webpanobject.className.includes("isWeb")) && isFloorp)))) {
          webpanobject.remove()
          webpanobject = null
        }
        if (webpanobject == null) {
  ...
  if (Services.prefs.getBoolPref("floorp.browser.sidebar.enable", false)) {
    (async() => {
      ...
      addContextBox("bsb-context-add", "bsb-context-add", "fill-login", `
           ...
           `, "context-viewsource", function () {
        document.getElementById("bsb-context-add").hidden = document.getElementById("context-viewsource").hidden || !document.getElementById("context-viewimage").hidden
      })
      addContextBox("bsb-context-link-add", "bsb-context-link-add", "context-sep-sendlinktodevice", `
           ...
           `, "context-openlink", function () {
        document.getElementById("bsb-context-link-add").hidden = document.getElementById("context-openlink").hidden
      })
